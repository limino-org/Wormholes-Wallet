<template>
    <div v-if="show" class="dialog-warning-dark">
      <div v-if="show" class="dialog-warning">
        <div class="dialog-warning-header"  style="color:000;">{{t('bourse.closeTit')}} </div>
        <div class="warning-icon">
            <van-icon color="#3AAE55" name="checked" size="35" />
        </div>
        <div class="warning-text">
            <span>{{t('bourse.tip10')}}</span>
        </div>
        <div class="footer-btns">
            <van-button style="width: 175px;" type="primary" class="btn" :disabled="Time !== 0" round @click="emitWarningSuccess">{{t('bourse.toHomePage')}}{{Time === 0 ? '' : `(${Time}s)`}}</van-button>
      </div>
    </div>
    </div>
</template>
